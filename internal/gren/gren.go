...
func New(optimize bool) api.Plugin {
	return api.Plugin{Name: "gren", Setup: setup(optimize)}
}

func setup(optimize bool) func(build api.PluginBuild) {
	return func(build api.PluginBuild) {
		build.OnResolve(api.OnResolveOptions{Filter: `\.gren$`}, onResolve)
		build.OnLoad(api.OnLoadOptions{Filter: `.*`, Namespace: "gren"}, onLoad(optimize))
	}
}
